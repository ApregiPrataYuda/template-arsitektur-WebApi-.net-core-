...
public class SubMenuController : BaseApiController
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(SubMenuCreateDto dto)
    {
        var result = await _service.CreateAsync(dto);
        return CreatedResponse(nameof(GetById), new { id = result.IdSubMenu }, result, "Sub Menu berhasil disimpan");
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, SubMenuUpdateDto dto)
    {
        var result = await _service.UpdateAsync(id, dto);
        return result == null
            ? NotFoundResponse()
            : SuccessResponse(result, "Sub Menu berhasil diupdate");
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        return success
            ? SuccessResponse<object>(new { }, "Sub Menu berhasil dihapus")
            : NotFoundResponse();
    }
}